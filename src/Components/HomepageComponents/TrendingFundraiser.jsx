...
const TrendingFundraiser = ({ currency, category, campaigns }) => {
  ...
  useEffect(() => {
    const loadCampaigns = async () => {
      try {
        ...
        // Use the campaigns passed as prop if available
        if (campaigns) {
          setStories(campaigns);
        }
        // Otherwise fetch all campaigns
        else {
          const response = await axios.get(`${BASE_URL}/api/campaigns/trending`);
          setStories(response.data);
        }
  ...
  //     try {
  //       setLoading(true);
  //       let endpoint = `${BASE_URL}/api/campaigns/get-all`;

  //       if (category) {
  //         endpoint = `${BASE_URL}/api/campaigns/getByCategory/${category}`;
  //       }
  ...
  //       if (isTrending) {
  //         // Fetch only trending campaigns
  //         endpoint = `${BASE_URL}/api/campaigns/trending`;
  //       } else {
  //         // Default to all campaigns or filtered by category
  //         endpoint = category
  //           ? `${BASE_URL}/api/campaigns/getByCategory/${category}`
  ...
  // added so can go the particular top fundraiser page from root
  ...
  return (
    <>
      <div className="trending-fundraiser-page">
        ...
        <div className="heading-border my-3 mx-auto  mb-4">
        </div>
        <Carousel
            ...
            >
              <div className="img-wrapper">
                <img
                  ...
                  src={
                    story.documents?.find(doc => doc.doc_type === "fundraiserImage")?.doc_url || "https://thumbs.dreamstime.com/b/image-not-available-icon-vector-set-white-background-eps-330821927.jpg"
                  } alt={story.title}
                  width="100%"
                  height="170"
              ...
              <div className="content-section padding-home">
                <div className="title" onClick={() => handleCardClick(story.id)}>{story.title}</div>
                ...
                <div className="name">by {story.name}</div>
              </div> */}
              ...
              <div className="share-buttons">
                ...
                <button className="btn-fb" style={{ background: "#d54400" }}
...
export default TrendingFundraiser;
